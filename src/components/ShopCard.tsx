...
export default function ShopCard({ shop }: { shop: Shop }) {
  return (
    <Link
      href={`/shops/${shop.id}`}
      className="group block bg-[var(--color-cream-light)] rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.4)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.5)] transition-all duration-300 hover:-translate-y-1 overflow-hidden border border-[var(--color-border)] hover:border-caramel/40"
    >
      {/* Header with emoji */}
      <div className="bg-gradient-to-r from-[var(--color-cream-light)] via-[var(--color-cream-light)] to-[var(--color-cream-light)] p-5 pb-4">
